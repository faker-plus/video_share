...
var authModal = new Vue({
    el: "#authAddedModal", methods: {
        searchFileInAuth: function () {
            ...
            if (isEmpty(file)) {
                alerts("Content cannot be empty");
            } else {
...
Vue.component('file-filter-item', {
    ...
    '                    <input type="text" title="Specify classification" data-toggle="tooltip" class="form-control file-filter category-filter"' +
    '                           placeholder="Classification name" onkeyup="fileFilter();"/>' +
    '                </div>' +
    '                <div class="col-sm-3 col-4">' +
    '                    <input type="text" title="Specify user" data-toggle="tooltip" class="form-control file-filter user-filter"' +
    '                           placeholder="User name or email" onkeyup="fileFilter();"/>' +
    '                </div>' +
    '                <div class="col-sm-4 col-4">' +
    '                    <input type="text" title="Specify File" data-toggle="tooltip" class="form-control file-filter file-name-filter"' +
    '                           placeholder="File name" onkeyup="fileFilter();"/>' +
    '                </div>' +
    '            </div>' +
    '            <br/>' +
    '        </div>'
...
function changeTabInfo(tabId) {
    ...
    if (tabId === "upload-manager") {
        ...
        alerts("No executable method found");
    }
    setTimeout(function () {
        $('[data-toggle="tooltip"]').tooltip();
        setCSS();
    ...
    $.ajax({
        url: "/category/" + categoryId, type: "DELETE", success: function (data) {
            ...
            if (json.status === "success") {
                ...
                alerts("Deletion failed. Please try again later");
            }
        }, error: function () {
            alerts("Deletion failed. The classification is already referenced by multiple files and cannot be deleted (try modifying the file classification or deleting the file).");
        }
    });
}
...
/**
 * Demonstrating Modal Boxes of the Same Type
 * @param table tableId
...
function editCategory() {
    $("#category-title").text("Edit Classification");
    var srcTr = $(event.srcElement).parents("tr");
    var categoryId = $(srcTr).children("td.category-id").text();
...
function saveCategory() {
    ...
    if (isEmpty(name)) {
        alerts("Category name cannot be empty");
    } else {
...
function updateAuth(url, down, uplo, dele, upda, visi, key) {
    ...
    $.ajax({
        url: url, type: "PUT", data: {auth: auth}, success: function (data) {
            ...
            alerts("Update" + boolToChinese(result));
            if (result) {
...
function doDelete(table, json, index, url) {
    ...
    if (selectedRows.length < 1) {
        ...
        layer.confirm('Are you sure to delete all selected rows', {
            btn: ['Confirm', 'Cancel']
        }, function () {
            var ids = "";
            ...
            $.ajax({
                url: url + ids, type: "DELETE", success: function (data) {
                    ...
                    if (json.status === "success") {
                        ...
                        alerts("Delete failed");
                    }
                }
            })
        }, function () {
            layer.msg("Canceled operation");
        });
    }
}
...
$(document).ready(function () {
    $("#auth-add-button").click(function () {
        ...
        if (isEmpty(files) || isEmpty(users)) {
            ...
            $.post("/auth", {files: files, users: users, auths: auths}, function (data) {
                ...
                alerts("Add" + boolToChinese(data.indexOf("success") > 0));
            })
        }
    });
    ...
    $("#user-password-update-button").click(function () {
        ...
        if (isEmpty(password)) {
            alerts("Password cannot be empty");
        } else {
            var user = app.users[$(selectedRows[rowIndex]).children(".user-index").attr("data-key")];
            layer.load(1);
            $.ajax({
                ...
                success: function (data) {
                    layer.closeAll();
                    alerts("Update" + boolToChinese(data.indexOf("success") > 0));
                }
            });
        }
    });
        ...
        if (app.selectedServerFiles.length > 0) {
            ...
            $.post("/file/server/share", {
                ...
                if (json.status === "success") {
                    ...
                    layer.msg("Successfully shared file");
                } else {
                    alerts("Sharing failed (possibly due to the existing local path or access link)");
                }
            });
        } else {
            alerts("No files have been selected yet");
        }
    });
    ...
    $(".to-upload-button").click(function () {
        layer.prompt({title: 'Please enter the file prefix to support rules', formType: 0}, function (prefix, index) {
            layer.close(index);
    ...
    $("#file-modify-button").click(function () {
        ...
        if (isEmpty(localUrl) && isEmpty(visitUrl)) {
            alerts("The content is empty and cannot be updated");
        } else {
            layer.load(1);
            $.ajax({
                ...
                success: function (data) {
                    ...
                    alerts((isEmpty(localUrl) ? "" : "Update local path" + boolToChinese(json.status.localUrl)) + (isEmpty(visitUrl) ? "" : (isEmpty(localUrl) ? "" : "，") + "Update access link" + boolToChinese(json.status.visitUrl)));
                },
                error: function () {
                    alerts("Connection to server exception");
                }
            });
        }
    });
