var footer = "<nav class='navbar navbar-default fixed-bottom justify-content-center' style='z-index:-1;'><div class='navbar-inner navbar-content-center text-center'><p>© 2023 Tian Copyright.</p></div></nav>";
...
function responseTip(data) {
    ...
    if (json.status === "success") {
        ...
        alerts("Saving failed. Please try again later");
    }
}
...
function sendVerifyCode(email, eventSrc) {
    if (isEmail(email)) {
        ...
        $.post("/common/" + email + "/code", function (data) {
            ...
            var json = JSON.parse(data);
            if (json.status === "success") {
                layer.msg("Successfully sent, please go to your email to check");
                ...
                alerts("Failed to obtain verification code. Please contact the administrator");
            }
        });
    } else {
        alerts("The email format is illegal");
    }
}
