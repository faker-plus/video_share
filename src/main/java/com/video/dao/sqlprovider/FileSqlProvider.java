...
public class FileSqlProvider {
    ...
    /**
     * Generate a query statement for basic information of a file
     *
    ...
    public String getBasicBy(@Param("userId") int userId, @Param("fileId") long fileId, @Param("fileName") String
        ...
        String sql = new SQL() {{
            ...
            ORDER_BY("f." + VideoApplication.settings.getStringUseEval(ConfigConsts.FILE_ORDER_BY_OF_SETTING));
        }}.toString();
        int size = VideoApplication.settings.getIntegerUseEval(ConfigConsts.FILE_PAGE_SIZE_OF_SETTING);
        return sql + " limit " + (offset * size) + "," + size;
    }

    private String getSqlEnds(int offset, String orderBy, String search) {
        int size = VideoApplication.settings.getIntegerUseEval(ConfigConsts.FILE_PAGE_SIZE_OF_SETTING);
        return getSearch(search) + " order by " + (Checker.isEmpty(orderBy) ? VideoApplication.settings
