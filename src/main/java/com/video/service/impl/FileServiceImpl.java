...
import com.video.entity.File;
import com.video.entity.User;
import com.video.model.AuthRecord;
import com.video.model.BaseAuthRecord;
import com.video.model.FileBasicRecord;
import com.video.model.FileRecord;
...
import com.video.dao.FileDAO;
import com.video.util.AzureBlob;
...
public class FileServiceImpl implements IFileService {
    ...
    public boolean updateFileInfo(long id, User user, String name, String category, String tag, String description) {
        ...
        if (Checker.isNotNull(file) && file.getIsUpdatable() == 1) {
            ...
            boolean canUpdate = (Checker.isNull(authRecord) ? user.getIsUpdatable() == 1 :
                    authRecord.getIsUpdatable() == 1) && Checker.isNotEmpty(name) && Pattern.compile(VideoApplication.settings.getStringUseEval(ConfigConsts.FILE_SUFFIX_MATCH_OF_SETTING)).matcher(suffix).matches();
            if (canUpdate) {
    ...
    public String getResource(String visitUrl, HttpServletRequest request) {
        ...
        boolean downloadable = VideoApplication.settings.getBooleanUseEval(ConfigConsts
    ...
    public boolean upload(int categoryId, String tag, String description, String prefix, MultipartFile multipartFile,
        ...
        if (user.getIsUploadable() == 1) {
            ...
            long maxSize = Formatter.sizeToLong(VideoApplication.settings.getStringUseEval(ConfigConsts
                    .FILE_MAX_SIZE_OF_SETTING));
            long size = multipartFile.getSize();
            boolean fileExists = localUrlExists(localUrl);
            //检测标签是否合法
            if (VideoApplication.settings.getBooleanUseEval(ConfigConsts.TAG_REQUIRE_OF_SETTING)) {
                String[] tags = Checker.checkNull(tag).split(ValueConsts.SPACE);
                if (tags.length <= VideoApplication.settings.getIntegerUseEval(ConfigConsts.TAG_SIZE_OF_SETTING)) {
                    int maxLength = VideoApplication.settings.getIntegerUseEval(ConfigConsts.TAG_LENGTH_OF_SETTING);
                    for (String t : tags) {
            ...
            boolean canUpload = !multipartFile.isEmpty() && size <= maxSize && Pattern.compile(VideoApplication
                    .settings.getStringUseEval(ConfigConsts.FILE_SUFFIX_MATCH_OF_SETTING)).matcher(suffix).matches()
                    && (Checker.isNotExists(localUrl) || !fileExists || VideoApplication.settings.getBooleanUseEval
            ...
            if (canUpload) {
                String visitUrl = getRegularVisitUrl(Checker.isNotEmpty(prefix) && user.getPermission() > 1 ? prefix
                        : VideoApplication.settings.getStringUseEval(ConfigConsts.CUSTOM_LINK_RULE_OF_SETTING), user,
                ...
                try {
                    ...
                    String visitUrlStringAll = AzureBlob.uploadFile(localFile, visitUrl);

                    if (visitUrlExists(visitUrlStringAll)) {
                        removeByVisitUrl(visitUrlStringAll);
                    }

                    logger.info("local url of upload file: " + localUrl);
                    logger.info("visit url of upload file: " + visitUrlStringAll);
                    File file = new File(name, suffix, localUrl, visitUrlStringAll, WebUtils.scriptFilter(description),
